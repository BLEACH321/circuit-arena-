...
import { Gavel, ShoppingBag, LogOut, CheckCircle, ShieldAlert, Cpu } from 'lucide-react';
...
export const TeamPortal: React.FC<TeamPortalProps> = ({
  ...
  return (
    <div className="min-h-screen bg-[#050508] text-slate-100 font-mono text-xs text-left p-6">
      ...
      {/* Top Banner Nav */}
      <div className="max-w-5xl mx-auto flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-800 pb-4 mb-6">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-[#0e111a] border border-[#00f0ff] rounded-lg flex items-center justify-center shadow-[0_0_15px_rgba(0,240,255,0.2)]">
            <Cpu className="w-5 h-5 text-[#00f0ff]" />
          </div>
          <div>
            <h1 className="font-display font-black text-xl tracking-wide uppercase text-white">
              CIRCUIT <span className="text-[#00f0ff] text-glow-cyan">ARENA</span>
            </h1>
        ...
        <button
          onClick={onLogout}
          className="px-4 py-2 bg-[#0e111a] border border-slate-800 hover:border-[#ff1a40] text-slate-400 hover:text-white rounded uppercase font-bold flex items-center gap-1.5"
        >
      ...
      <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        {/* LEFT COLUMN: TEAM HUD (BUDGET & INFO) */}
        <div className="space-y-6 md:col-span-1">
          
          {/* Team Profile */}
          <div className="glass-panel p-5 rounded-xl border border-slate-800 space-y-4">
            <div className="border-b border-slate-800/80 pb-2.5">
              <span className="text-[9px] text-[#00f0ff] uppercase block font-bold">// SQUAD telemetry</span>
              <strong className="text-sm font-display font-bold text-white uppercase block mt-1">{myTeam.teamName}</strong>
              <span className="text-[9px] text-slate-500 block">{myTeam.college}</span>
            </div>

            <div className="p-4 bg-[#07080c] border border-slate-900 rounded-lg text-center relative overflow-hidden">
              ...
              <span className="text-3xl font-black font-display text-amber-500 text-glow-orange block mt-1.5">
                {myTeam.budget} <span className="text-xs font-mono text-white">COINS</span>
              </span>
            </div>

            <div className="text-[10px] text-slate-400 flex justify-between items-center bg-[#07080c]/50 p-2 rounded">
          ...
          <div className="glass-panel p-5 rounded-xl border border-slate-800 space-y-4">
            <h3 className="font-display font-black text-white uppercase text-sm tracking-wide flex items-center gap-2">
              <ShoppingBag className="w-4 h-4 text-[#00f0ff]" /> Acquired Inventory ({myTeam.inventory?.length || 0})
            </h3>

            <div className="space-y-2 max-h-[220px] overflow-y-auto pr-1">
              {!myTeam.inventory || myTeam.inventory.length === 0 ? (
                <div className="text-slate-700 text-center py-6">No items won yet.</div>
              ) : (
                myTeam.inventory.map((item: any, idx: number) => (
                  <div key={idx} className="p-2 bg-[#07080c] border border-slate-900 rounded flex justify-between items-center">
                    <div>
                      <strong className="text-slate-200 text-xs block">{item.name}</strong>
        ...
        <div className="space-y-6 md:col-span-2">
          ...
          {/* Active Item Panel */}
          <div className="glass-panel p-6 rounded-xl border border-[#ff6b00]/30 space-y-5 bg-[#0e111a]/40 shadow-lg relative overflow-hidden">
            <div className="absolute top-0 right-0 w-32 h-32 bg-[#ff6b00]/5 rounded-full blur-2xl pointer-events-none" />

            <div className="flex justify-between items-center border-b border-slate-800 pb-3">
              <div>
                <span className="text-[9px] text-[#ff6b00] uppercase font-bold tracking-widest block">// LIVE AUCTION BLOCK</span>
                <h3 className="text-lg font-black font-display text-white uppercase mt-0.5">CURRENT ITEM</h3>
              </div>
            ...
            {!activeItem ? (
              <div className="text-center py-16 space-y-3">
                ...
                <p className="text-slate-400 font-sans text-xs">Waiting for the organizer to activate the next component...</p>
              </div>
            ) : (
              <div className="space-y-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 items-center">
                  <div className="sm:col-span-2 space-y-1">
                    ...
                    <div className="flex gap-4 pt-1 font-mono text-[9px] text-slate-400">
                      <span>Starting Price: <strong className="text-white">{currentItem.basePrice} Coins</strong></span>
                      <span>Min Increment: <strong className="text-white">+{currentItem.minIncrement} Coins</strong></span>
                    </div>
                  </div>

                  {/* Countdown display */}
                  <div className="bg-[#07080c] p-2.5 border border-slate-800 rounded text-center">
                ...
                <div className="grid grid-cols-2 gap-4 bg-[#07080c] p-4 rounded-xl border border-slate-900">
                  <div>
                  ...
                  <div>
                    ...
                    {isHighestBidder ? (
                      <span className="text-xs font-bold text-[#00ff66] flex items-center gap-1.5 mt-2 bg-[#00ff66]/10 px-2 py-1.5 border border-[#00ff66]/30 rounded">
                        <CheckCircle className="w-3.5 h-3.5 text-[#00ff66]" /> YOU HOLD HIGHEST
                      </span>
                    ) : (
                      <span className="text-xs font-bold text-slate-300 block mt-2 truncate px-2 py-1.5 bg-slate-900 border border-slate-800 rounded">
                ...
                <button
                  ...
                  className={`w-full py-5 rounded-xl font-display font-black text-sm tracking-widest uppercase transition-all flex flex-col items-center justify-center gap-1 shadow-lg ${
                    isBidDisabled
                      ? 'bg-slate-900 border border-slate-800/80 text-slate-600 cursor-not-allowed'
                      : 'bg-gradient-to-r from-[#ff6b00] to-[#ff3c00] hover:scale-[1.01] active:scale-[0.99] text-black shadow-[0_0_25px_rgba(255,107,0,0.4)] cursor-pointer'
                  }`}
                >
                ...
                {isHighestBidder && (
                  <p className="text-[#00ff66] text-[10px] text-center font-sans">
                    You are currently the highest bidder. Bidding again is locked unless you are outbid.
                  </p>
                )}
                {!hasEnoughBudget && currentItem && (
                  <p className="text-red-500 text-[10px] text-center font-sans font-bold">
                ...
                <div className="space-y-2">
                  <span className="text-[9px] text-slate-500 uppercase tracking-wider block font-bold">BID HISTORY FEED</span>
                  <div className="bg-black/70 p-3 rounded border border-slate-900 h-[100px] overflow-y-auto space-y-1 font-mono text-[9px]">
                    {activeItem.bidsLog.length === 0 ? (
                      <div className="text-slate-800 text-center py-6">No bids placed on this item.</div>
                    ) : (
